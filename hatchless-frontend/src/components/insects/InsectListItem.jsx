...
const InsectListItem = ({ insect }) => {
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" mb={8} withBorder className="insect-list-card mb-15">
      <div className="insect-list-item">
        ...
        <div className="margin-bottom">
          <Text color="dimmed" className="italic">{insect.scientific_name}</Text>
        </div>
          ...
          <div>
            {insect.fly_patterns.map((fly, index) => (
              <div key={index} className="flex row space-between">
                ...
                <Text size="xs" color="dimmed">{fly.category}</Text>
              </div>
            ))}
          </div>
        ) : (
